...
export class Dashboard implements OnInit {
  ...
  loadRequests() {
    ...
    this.http.get<any[]>(`${this.apiUrl}/quoterequest/agent`).subscribe({
      next: data => {
        this.allRequests.set(data ?? []);
      },
      error: () => { this.allRequests.set([]); },
  ...
  loadQuotes() {
    ...
    this.http.get<any[]>(`${this.apiUrl}/quote/agent`).subscribe({
      next: data => {
  ...
  loadRecommendations() {
    ...
    this.http.get<any[]>(`${this.apiUrl}/recommendation/agent`).subscribe({
  ...
  loadCommissions() {
    ...
    this.http.get<any[]>(`${this.apiUrl}/payment/agent`).subscribe({
      next: data => {
  ...
  submitRecommendation() {
    ...
    this.http.post<any>(`${this.apiUrl}/recommendation`, {
      quoteRequestId: this.selectedRecRequest.id,
      agentMessage: this.recommendationMessage
    }).subscribe({
      next: res => {
        this.showToastMsg(`✓ Recommendation sent to ${this.selectedRecRequest.companyName}!`, 'success');
